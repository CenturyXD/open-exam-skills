# ชุดเรียน: พ.ร.บ.ระเบียบบริหารราชการศาลยุติธรรม พ.ศ. 2543 และที่แก้ไขเพิ่มเติม

ชุดทบทวนสอบจากตัวบทและแหล่งราชการ (ราชกิจจานุเบกษา, สำนักงานศาลยุติธรรม)

## ไฟล์ในชุดนี้

| ไฟล์ | คำอธิบาย |
|------|----------|
| `SUMMARY.md` / `summary.html` | **สรุปฉบับละเอียด (เริ่มที่นี่)** |
| `SOURCES.md` | แหล่งอ้างอิง |
| `mindmap.md` / `mindmap.html` | แผนที่ความคิด |
| `flashcards.json` / `flashcards.html` | บัตรคำ 20 ใบ |
| `quiz.json` / `quiz.html` | แบบทดสอบ 5 ชุด ชุดละ 30 ข้อ — เลือกชุดได้ |

## วิธีใช้

0. เปิด [เมนูหลัก](../index.html) แล้วเลือกอันที่ต้องการ
1. อ่าน `summary.html`
2. ดูโครงใน `mindmap.html`
3. ท่องด้วย `flashcards.html`
4. ทดสอบด้วย `quiz.html`

### อ่านบนมือถือ (Cloud)

- **เมนูหลัก:** https://htmlpreview.github.io/?https://github.com/CenturyXD/open-exam-skills/blob/cursor/court-justice-admin-act-7242/study/index.html
- สรุป: https://htmlpreview.github.io/?https://github.com/CenturyXD/open-exam-skills/blob/cursor/court-justice-admin-act-7242/study/court-of-justice-admin/summary.html
- แผนที่ความคิด: https://htmlpreview.github.io/?https://github.com/CenturyXD/open-exam-skills/blob/cursor/court-justice-admin-act-7242/study/court-of-justice-admin/mindmap.html
- บัตรคำ: https://htmlpreview.github.io/?https://github.com/CenturyXD/open-exam-skills/blob/cursor/court-justice-admin-act-7242/study/court-of-justice-admin/flashcards.html
- ข้อสอบ: https://htmlpreview.github.io/?https://github.com/CenturyXD/open-exam-skills/blob/cursor/court-justice-admin-act-7242/study/court-of-justice-admin/quiz.html

## สร้าง HTML ใหม่

```bash
cd skills/mindmap && python main.py -i ../../study/court-of-justice-admin/mindmap.md -o ../../study/court-of-justice-admin/mindmap.html
cd ../flashcards && python main.py -i ../../study/court-of-justice-admin/flashcards.json -o ../../study/court-of-justice-admin/flashcards.html
cd ../quiz && python main.py -i ../../study/court-of-justice-admin/quiz.json -o ../../study/court-of-justice-admin/quiz.html
```

## ขอบเขตเนื้อหา

- พ.ร.บ.ระเบียบบริหารราชการศาลยุติธรรม พ.ศ. 2543 (ฉบับหลัก)
- การแก้ไขเพิ่มเติม ฉบับที่ 2 พ.ศ. 2551 · ฉบับที่ 3 พ.ศ. 2558 · ฉบับที่ 4 พ.ศ. 2562
- สำนักงานศาลยุติธรรม · เลขาธิการ · ก.บ.ศ. · ก.ศ. · การโอนอำนาจจากกระทรวงยุติธรรม
